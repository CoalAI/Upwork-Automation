...
const JobsPage: React.FC<JobsPageProps> = ({ apiUrl }) => {
  ...
  const fetchJobsDataRef = useRef<typeof fetchJobsData>();
  const startPollingRef = useRef<typeof startPolling>();
  const stopPollingRef = useRef<typeof stopPolling>();
  ...
  const fetchJobsData = useCallback(async (url: string) => {
    ...
    try {
      ...
      setJobs(sampleJobs);
      setApiError(true);
      // Use toast directly instead of depending on it
  ...
  // Update refs when functions are recreated
  useEffect(() => {
    fetchJobsDataRef.current = fetchJobsData;
  }, [fetchJobsData]);
  ...
  }, [apiUrlToUse, notificationsEnabled]);
  ...
  const startPolling = useCallback(() => {
    ...
    setNextPollTime(new Date(Date.now() + 90000));
    return interval;
  }, [relevanceFilter, notificationsEnabled, fetchJobsByRelevance, pollingInterval]);
  ...
  }, []); // Remove fetchJobsByRelevance from dependencies
  ...
  }, [relevanceFilter, apiUrlToUse]); // Remove fetchJobsByRelevance and fetchJobsData from dependencies
  ...
  }, [relevanceFilter, notificationsEnabled]); // Remove startPolling and stopPolling from dependencies
  ...
  }, []); // Remove stopPolling from dependencies
  ...
  const calculateAnalytics = useCallback((jobsData: any[]) => {
